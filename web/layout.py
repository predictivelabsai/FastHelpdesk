"""FastHelpdesk 3-pane layout: top bar · left nav · center · right AI rail.
...
# FastHelpdesk palette — teal/support, amber warn, rose breach.
LAYOUT_CSS = """
:root{
  --bg:#f3f8f8; --surface:#ffffff; --surface-2:#e9f1f1; --border:#dbe7e7; --text:#16231f;
  --text-dim:#4b5a55; --text-mute:#869691; --accent:#0d9488; --accent-hover:#0b7d73;
  --accent-light:#d4f0ec; --ok:#16a34a; --ok-light:#dcfce7; --warn:#d97706; --warn-light:#fef3c7;
  --breach:#e11d48; --breach-light:#ffe4e6;
}
...
.kpi.breach::after{background:var(--breach);} .kpi.warn::after{background:var(--warn);} .kpi.ok::after{background:var(--ok);}
...
.pill.open{background:#dbeafe;color:#1d4ed8;} .pill.replied{background:var(--accent-light);color:var(--accent-hover);}
.pill.onhold{background:var(--warn-light);color:#92400e;} .pill.resolved{background:var(--ok-light);color:#166534;} .pill.closed{background:#e2e8f0;color:#475569;}
.pill.urgent{background:var(--breach-light);color:#9f1239;} .pill.high{background:#ffedd5;color:#9a3412;} .pill.medium{background:var(--warn-light);color:#92400e;} .pill.low{background:#e2e8f0;color:#475569;}
.sla{display:inline-block;padding:2px 9px;border-radius:6px;font-size:11px;font-weight:600;white-space:nowrap;}
.sla.ok{background:var(--ok-light);color:#166534;} .sla.warn{background:var(--warn-light);color:#92400e;} .sla.breach{background:var(--breach-light);color:#9f1239;} .sla.neutral{background:var(--surface-2);color:var(--text-mute);}
.funnel-row{display:grid;grid-template-columns:130px 1fr 70px;align-items:center;gap:10px;margin-bottom:7px;font-size:13px;}
...
.bubble{border:1px solid var(--border);border-radius:10px;padding:10px 14px;max-width:82%;font-size:13px;line-height:1.5;}
.bubble .who{font-size:11px;color:var(--text-mute);margin-bottom:3px;font-weight:600;}
.bubble.customer{align-self:flex-start;background:var(--surface);}
.bubble.agent{align-self:flex-end;background:var(--accent-light);border-color:var(--accent-light);}
.bubble.note{align-self:stretch;background:var(--warn-light);border-color:#fde68a;color:#92400e;font-style:italic;max-width:100%;}
.timeline{list-style:none;padding:0;margin:0;}
.timeline li{padding:8px 0 8px 20px;border-left:2px solid var(--border);position:relative;margin-left:4px;}
...
.reply-form{margin-top:12px;border-top:1px solid var(--border);padding-top:12px;}
.reply-form textarea{width:100%;min-height:80px;padding:10px 12px;border:1px solid var(--border);border-radius:8px;font-size:13px;font-family:inherit;resize:vertical;}
.reply-form textarea:focus{border-color:var(--accent);outline:none;box-shadow:0 0 0 3px var(--accent-light);}
.kb-card{background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:14px 16px;margin-bottom:10px;}
.kb-card h4{margin:0 0 4px;font-size:14px;} .kb-card .meta{color:var(--text-mute);font-size:12px;}
.login-wrap{height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#e6f3f1 0%,#d4f0ec 100%);}
...
SAMPLE_QUESTIONS = [
    ...
    "What's the busiest team right now?",
    "Summarise today's support load.",
]
...
def topbar(env, user_email):
    ...
    return Div(Div(Span(cls="brand-dot"), Span("Fast", style="font-weight:800;"),
                   Span("Helpdesk", style="color:var(--accent);font-weight:700;letter-spacing:.5px;"), cls="brand"),
               right, cls="topbar")
...
def right_pane_chat(thread_id):
    return Div(
        Div(H3("AI Assistant"),
            ...
            cls="right-header"),
        Div(Div(P("Ask about tickets, SLAs, agents or the knowledge base — or tap a question below.",
            ...
            Form(Input(type="hidden", name="thread_id", value=thread_id, id="thread-id"),
                 Div(Input(type="text", name="message", id="chat-input",
                           placeholder="Ask a question or /sla /tickets /help …", autocomplete="off"),
...
function toggleCopilot(){var app=document.querySelector('.app');if(!app)return;app.classList.toggle('right-collapsed');
  ...
  try{localStorage.setItem('hdCollapsed',app.classList.contains('right-collapsed')?'1':'0');}catch(e){}_sync();}
function toggleExpand(){var app=document.querySelector('.app');if(!app)return;app.classList.remove('right-collapsed');app.classList.toggle('right-expanded');
  try{localStorage.setItem('hdExpanded',app.classList.contains('right-expanded')?'1':'0');localStorage.setItem('hdCollapsed','0');}catch(e){}_sync();}
(function(){try{var app=document.querySelector('.app');if(!app)return;
  if(localStorage.getItem('hdCollapsed')==='1')app.classList.add('right-collapsed');
  else if(localStorage.getItem('hdExpanded')==='1')app.classList.add('right-expanded');}catch(e){}})();
